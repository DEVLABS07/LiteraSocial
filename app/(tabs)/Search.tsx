...
export default function Search() {
    ...
    const handle_search = async () => {
        ...
        try {
            ...
            if (!response) {
                setLoader(false);
                setData({ heading: "No result found", UserId: "Invalid Search", Username: "Error" });
                return;
            }
            setLoader(false);
    ...
    return (
        <View style={{ flex: 1, backgroundColor: "white", display: "flex", alignItems: 'center', paddingTop: 50 }}>
            ...
            <TextInput onSubmitEditing={handle_search} value={search} onChangeText={setSearch} style={{ width: "90%", height: 50, borderWidth: 1, borderColor: "gray", borderRadius: 10, padding: 10, paddingLeft: 50, paddingRight: 20 }} placeholder="Search for Poems, Stories, Users..." />
                        ...
                        <Pressable style={{ minWidth: "95%", maxWidth: "95%", width: "110%", borderWidth: 1, borderColor: 'lightgray', display: "flex", alignItems: 'center', borderRadius: 10, marginTop: 20, }}>
                            <View style={{ left: 10, width: "100%", padding: 10, display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: "flex-start", }}>
                                ...
                                <View style={{ display: 'flex', flexDirection: option == "l" ? "column" : "row", alignItems: option != "l" ? 'center' : "flex-start", justifyContent: "center", padding: 10, gap: 10 }}>
                                    ...
                                    {option != "l" ? <Pressable style={{ padding: 8, marginLeft: 20, borderRadius: 10, borderWidth: 1, borderColor: "#405DE6" }}><Text style={{ color: "#405DE6" }}>Follow</Text></Pressable>
                                        : null}
                                </View>
